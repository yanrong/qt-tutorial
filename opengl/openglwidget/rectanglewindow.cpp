...
{
    //set the current rendering contex
    makeCurrent();
...
void RectangleWindow::initializeGL()
{
    ...
    //this function just call once when window is first show or first renderned

    //crate shader program
    m_program = new QOpenGLShaderProgram();

    //read shader source from local files
    if (!m_program->addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/openglwidget.vert")){
            qDebug() << "vertex shader errors:\n" << m_program->log();
    }

    if (!m_program->addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/openglwidget.frag")){
    ...
    m_vertexBuffer.resize(2 * 3 * 4);
    GLfloat* buf = m_vertexBuffer.data();
    ...
    m_vbo.bind();
    m_vbo.allocate(m_vertexBuffer.data(), m_vertexBuffer.size() * sizeof(GLfloat));
...
void RectangleWindow::paintGL()
{
    ...
    //already set the VAO status, if we want to use it, just bind it to current context
    m_vao.bind();
...
{
    //for update color
    //get the buffer data address and update it's color value
    float *buf = m_vertexBuffer.data();
    for (int i = 0; i < 4; i++, buf += 6) {
        buf[3] = m_vertexColors[i].redF();
        buf[4] = m_vertexColors[i].greenF();
        buf[5] = m_vertexColors[i].blueF();
